...
class FetchProfileDataBloc extends Bloc<FetchProfileBlocEvent, FetchProfileBlocState> {
  ...
  FetchProfileDataBloc({required this.profileRepository, required this.profileDataBloc}) : super(FetchProfileBlocInitial()) {
    on<FetchProfileData>((event, emit) async {
      String userId = FirebaseAuth.instance.currentUser?.uid ?? '';
      if(userId.isNotEmpty){
        emit(FetchProfileBlocLoading());
        final response = await profileRepository.getProfileData(userId: userId);
        switch(response) {
