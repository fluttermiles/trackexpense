...
class RupeeDataBloc extends Bloc<RupeeDataBlocEvent, RupeeDataBlocState> {
  ...
  RupeeDataBloc({required this.rupeeObjectRepository}) : super(RupeeDataBlocInitial()) {
    on<RupeeData>((event, emit) async {
      emit(RupeeDataBlocLoading());
      final response = await rupeeObjectRepository.getRupeeDataByFilters(day: event.selectedDate.day, month: event.selectedDate.month, year: event.selectedDate.year);
      switch(response) {
